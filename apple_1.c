...
{
	return (BYTE(6) << 10 | BYTE(5) << 2 | GET_SET_BIT(BYTE(4), 6, 2));
}
...
static int16_t get_x_delta(const uint8_t* buf) {
	uint16_t num = ((GET_SET_BIT(BYTE(4), 2, 2) << 8) | BYTE(2));
...
static int16_t get_y_delta(const uint8_t* buf) {
	uint16_t num = ((GET_SET_BIT(BYTE(4), 4, 2) << 8) | BYTE(3));
